...
const { data: page } = await useAsyncData(path.value, async () =>
  await queryCollection(collection.value).path(path.value).first() as Collections['articles_bg'], 
)
  ...
  <div v-if="page">
    ...
    <article class="writing mx-auto px-4 sm:max-w-2xl md:max-w-3xl lg:max-w-4xl">
      ...
      <div class="info-section mt-1 flex flex-col gap-2 sm:flex-row sm:gap-4">
        ...
        <p>{{ page?.readingTime }} {{ page?.readingTime === '1' ? $t("writing.readingTimeSingle") : $t("writing.readingTime") }}</p>
        <p class="hidden sm:block">
          |
        </p>
        <UTooltip
